  ...
  return (
    <article className="gallery__item">
      <div className="gallery__item-wrapper">
        ...
        />
        {isOwn && <button className="gallery__delete-button" type="button" onClick={() => onDelete(card._id) }/>}
      </div>
      <h2 className="gallery__title">{card.name}</h2>
      <div className="gallery__like-wrapper">
        <button className={`gallery__like-button ${isLiked ? 'gallery__like-button_active' : ''}`} type="button" onClick ={handleLikeClick} />
